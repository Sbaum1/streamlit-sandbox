...
AUDIO_PATH = Path("forecastiq/assets/audio/sentinel_orientation.mp3")

SESSION_KEYS = {
    "seen": "sentinel_landing_seen",
    "voice_played": "sentinel_orientation_voice_played",
    "voice_muted": "sentinel_orientation_voice_muted",
}
...
def _render_voice_briefing():
    st.markdown("#### 🎙️ Sentinel Orientation")

    cols = st.columns([1, 3, 1])

    with cols[0]:
        if st.button("▶ Play", key="sentinel_orientation_play"):
            st.session_state[SESSION_KEYS["voice_played"]] = True

    with cols[1]:
        st.caption(
            "An executive briefing explaining how Sentinel works — and what it does not do."
        )
