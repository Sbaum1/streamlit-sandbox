...
def run_ensemble_consensus_reference(
    ...
    if not model_outputs:
        raise ValueError(
            "Ensemble Consensus Reference requires at least one model output."
        )
    ...
    if len(horizons) != 1:
        raise ValueError(
            "All model outputs must share the same forecast horizon."
        )

    horizon = horizons.pop()

    forecasts = [
        np.array(m.point_forecast, dtype=float)
    ...
    mean_forecast = np.mean(stacked, axis=0)
    median_forecast = np.median(stacked, axis=0)
    std_dev = np.std(stacked, axis=0)

    # Agreement score: lower dispersion = higher agreement
    ...
    diagnostics = {
        ...
        "notes": (
            "Consensus diagnostics only. "
            "No independent forecasting performed."
        ),
    }
