...
def _ai_insight(section_key: str, context: str, force: bool = False):
        ...
        with st.spinner("Auto-Intelligence generating insight…"):
            try:
                ...
                payload = {
                    ...
                    "max_tokens": 1000,
                    "system": (
                        "You are ForecastIQ's Auto-Intelligence engine. "
    ...
    if insight:
        st.markdown(
            f'''<div style="background:linear-gradient(135deg,rgba(124,58,237,.08),rgba(37,99,235,.06));
            ...
            text-transform:uppercase;color:#7c3aed;margin-bottom:.35rem">
            ⚡ Auto-Intelligence</div>
            <div style="font-family:DM Mono,monospace;font-size:.72rem;color:#c4b5fd;
...
[data-testid="stSidebar"]          { background: #0d1221; border-right: 1px solid #1e2a45; }
...
.kpi-tile:hover { border-color: #2563eb; transform: translateY(-2px); }
.kpi-label {
...
hr { border-color: #1e2a45 !important; margin: 1.75rem 0 !important; }

[data-testid="stFileUploadDropzone"] {
    background: #0d1221 !important;
    border: 1.5px dashed #1e2a45 !important;
    border-radius: 10px !important;
...
def _layout(title: str, sub: str = "", height: int = 420) -> dict:
    ...
    d["title"] = dict(
        text=(
            ...
            f"font-weight:700;color:#e2e8f0'>{title}</span>"
            + (f"<br><span style='font-size:10px;color:#4b5e80'>{sub}</span>" if sub else "")
        ),
...
def render_home():
    ...
    <div style="margin-bottom:1.75rem">
      <div style="font-family:'DM Mono',monospace;font-size:.6rem;letter-spacing:.22em;
                  text-transform:uppercase;color:#2563eb;margin-bottom:.3rem">
        ForecastIQ v2 · Executive Suite
      </div>
      <div style="font-family:'Syne',sans-serif;font-size:2rem;font-weight:800;
                  color:#e2e8f0;line-height:1.1">
        Decision Center
      </div>
      <div style="font-family:'DM Mono',monospace;font-size:.7rem;color:#4b5e80;
                  margin-top:.35rem">
        Commit your time series data · Configure the Sentinel Engine · Drive precision decisions
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.latest_forecast_df is None:
        st.markdown(
            ...
            '<span style="font-size:.85rem;color:#4b5e80">'
...
def _rolling(hist_df: pd.DataFrame):
    ...
    fig.add_trace(go.Scatter(x=df["date"], y=df["R3"], mode="lines",
        name="3-Period MA", line=dict(color="#2563eb", width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=df["date"], y=df["R12"], mode="lines",
        name="12-Period MA", line=dict(color="#7c3aed", width=2, dash="dash")), row=1, col=1)
...
def _seasonality(hist_df: pd.DataFrame):
    ...
    palette = ["#1e40af","#2563eb","#3b82f6","#60a5fa","#93c5fd",
               "#a78bfa","#7c3aed","#6d28d9","#4c1d95","#ddd6fe"]
    ...
    if len(years) > 1:
        ...
        hfig = go.Figure(go.Heatmap(
            ...
            colorscale=[[0,"#0f2040"],[0.5,"#2563eb"],[1,"#93c5fd"]],
...
def _distribution(hist_df: pd.DataFrame):
    ...
    with c2:
        fig2 = go.Figure(go.Box(
            y=vals, name="Values",
            marker_color="#2563eb", line_color="#60a5fa",
    ...
    for lbl, val in stats:
        html += (
            f'<div style="background:#0d1221;border:1px solid #1e2a45;border-radius:8px;'
            f'padding:.75rem;text-align:center">'
            f'<div style="font-family:DM Mono,monospace;font-size:.58rem;letter-spacing:.1em;'
            f'text-transform:uppercase;color:#4b5e80;margin-bottom:.2rem">{lbl}</div>'
            f'<div style="font-family:Syne,sans-serif;font-size:.9rem;font-weight:600;'
            f'color:#cbd5e1">{_fmt(val)}</div></div>'
        )
    html += "</div>"
...
def _hero_chart(hist_df: pd.DataFrame):
    ...
    fig.update_layout(**_layout(
        "Forecast Trajectory",
        f"Sentinel Engine v{ver} · {tier} Tier · Primary Ensemble (solid) · "
    ...
    st.markdown(
        '<div style="font-family:DM Mono,monospace;font-size:.62rem;color:#4b5e80;'
...
def _decomposition(hist_df: pd.DataFrame):
    ...
    fig.add_trace(go.Scatter(x=trend.index, y=trend.values, mode="lines",
        name=f"{window}-Period Trend", line=dict(color="#2563eb", width=2.5)), row=1, col=1)
    ...
    fig.add_trace(go.Histogram(x=residual.dropna().values, nbinsx=20,
        name="Residual Dist",
        marker=dict(color="#7c3aed", opacity=0.7)), row=3, col=1)
