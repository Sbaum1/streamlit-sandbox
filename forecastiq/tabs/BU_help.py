...
def render_help():
    ...
    <div class="help-hero">
      <div style="font-size:.55rem;letter-spacing:.2em;text-transform:uppercase;
                  color:#2563eb;margin-bottom:.4rem;font-family:'DM Mono',monospace">
        ForecastIQ v2 · Executive Suite · Help & Reference
      </div>
      <div style="font-family:'Syne',sans-serif;font-size:1.6rem;font-weight:800;
                  color:#e2e8f0;line-height:1.2;margin-bottom:.6rem">
        Platform Guide
      </div>
      <div style="font-family:'DM Mono',monospace;font-size:.75rem;color:#64748b;
                  line-height:1.7">
        Complete reference for every control, metric, chart, and workflow in ForecastIQ.
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    with nav[0]:
        _section("◎ What is ForecastIQ?", """
ForecastIQ is an enterprise-grade time series forecasting platform powered by the
Sentinel Engine — a 19-model ensemble that runs statistical, machine learning, and
...
4. **Run Forecast** — Click RUN FORECAST. The Sentinel Engine runs all models simultaneously.
...
ForecastIQ expects a two-column time series: one date column and one numeric value column.
...
The run signature records which data fingerprint was used for every forecast.
    ...
    with nav[2]:
        _section("◎ Model Tier", """
The Model Tier controls which models the Sentinel Engine runs.
...
ForecastIQ certifies every model against the M-Competition benchmark standard —
...
ForecastIQ supports two distinct methods for incorporating macroeconomic conditions
...
Requires: free FRED API key + sentinel engine exog support.

**Tier B — Macro Multipliers** (fully active today)
You specify assumed future changes in macro variables (e.g. +1.5% interest rates).
ForecastIQ applies calibrated economic multipliers to translate each assumption
...
sentinel engine update to accept exog_df as a parameter in run_all_models().
...
Auto-Intelligence connects ForecastIQ to Claude (Anthropic's AI) to generate
...
ForecastIQ is powered by the Sentinel Engine v2.0.0.

For platform issues, engine updates, or feature requests — consult your platform
engineer or administrator. The sentinel engine source files are located at the
repository root. The ForecastIQ application layer is in the forecastiq/ directory.

**File structure:**
- forecastiq/app.py — Main Streamlit entry point
- forecastiq/tabs/ — All tab render functions
- forecastiq/sidebar/ — Command center
- forecastiq/engine/ — Sentinel adapter + macro variables
- forecastiq/state/ — Session state management
- forecastiq/utils/ — Frequency inference, validation, hashing
        """)
...
def _multiplier_table():
    ...
    <div style="display:grid;grid-template-columns:1.8fr 1.2fr 2fr;gap:.3rem;
                ...
                color:#4b5e80;letter-spacing:.08em;text-transform:uppercase;
                padding:.3rem 0;border-bottom:1px solid #1e2a45;margin-bottom:.3rem">
    ...
    <div style="display:grid;grid-template-columns:1.8fr 1.2fr 2fr;gap:.3rem;
      ...
      <div style="color:#e2e8f0">{var}</div>
      <div style="color:#60a5fa;font-weight:600">{mult}</div>
      <div style="color:#64748b">{rationale}</div>
    </div>
    """
    html += "</div>"
