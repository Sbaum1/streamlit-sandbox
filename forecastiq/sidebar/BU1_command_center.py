...
_CSS = """
<style>
[data-testid="stSidebar"] { background:#0d1221!important; border-right:1px solid #1e2a45!important; }
...
.sb-zone {
    ...
    padding-bottom:.35rem; border-bottom:1px solid #1e2a45;
...
def render_command_center():
    ...
    with st.sidebar:
        ...
        # ── Brand ────────────────────────────────────────────────────────────
        st.markdown("""
        <div style="padding:.6rem 0 .4rem;border-bottom:1px solid #1e2a45;margin-bottom:.2rem">
          <div style="font-size:.55rem;letter-spacing:.22em;text-transform:uppercase;
                      color:#2563eb;margin-bottom:.15rem">ForecastIQ v2 · Sentinel</div>
          <div style="font-family:'Syne',sans-serif;font-size:.95rem;font-weight:800;
                      color:#e2e8f0">Command Center</div>
        </div>
        ...
        html = (
            ...
            + _row("Engine",  f"Sentinel v{st.session_state.sentinel_engine_version or '2.0.0'}", "blue")
        ...
        ai_on = st.toggle(
            ...
            help=(
                "When enabled, ForecastIQ uses Claude AI to generate plain-English "
        ...
        if ai_on:
            st.markdown(
                '<div style="font-size:.6rem;color:#7c3aed;padding:.25rem 0 .5rem">'
        ...
        if macro_mode == "Tier A — FRED Regressors":
            ...
            st.markdown(
                ...
                'training requires a sentinel engine update to accept exog_df. '
        ...
        # ── Tier B — Multipliers ──────────────────────────────────────────
            ...
            st.markdown(
                '<div style="font-size:.6rem;color:#4b5e80;margin:.15rem 0 .4rem">'
                'Set macro assumptions — ForecastIQ applies calibrated shocks to the baseline.</div>',
                unsafe_allow_html=True,
            )
            ...
            if tier_b_enabled:
                ...
                st.markdown(
                    f'<div style="background:#0d1221;border:1px solid #1e2a45;'
        ...
        # Footer
        st.markdown(
            '<div style="margin-top:2rem;padding-top:.75rem;border-top:1px solid #1e2a45;'
            'font-size:.55rem;color:#1e2a45;text-align:center;letter-spacing:.06em">'
            'ForecastIQ · Sentinel Engine v2.0.0<br>'
            'Executive Suite · Fortune 100 Edition'
            '</div>',
            unsafe_allow_html=True,
        )
...
def _run_forecast(tier, horizon, backtest, ci_level,
    ...
    with st.spinner("Sentinel Engine running…"):
        try:
