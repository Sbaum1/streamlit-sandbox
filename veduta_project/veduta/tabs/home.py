...
def _ai_insight(section_key: str, context: str, force: bool = False):
        ...
        with st.spinner("Auto-Intelligence generating insight…"):
            try:
                ...
                payload = {
                    ...
                    "max_tokens": 1000,
                    "system": (
                        "You are VEDUTA's Foresight Engine Auto-Intelligence. "
                        "You receive real data from an executive forecasting platform "
    ...
    if insight:
        st.markdown(
            f'''<div style="background:linear-gradient(135deg,rgba(200,151,74,.08),rgba(82,184,178,.06));
            border:1px solid rgba(200,151,74,.2);border-left:3px solid #C8974A;
            ...
            <div style="font-family:DM Mono,monospace;font-size:.72rem;color:#8FA3B8;
...
.exec-card {
    background: linear-gradient(135deg, #0D1420 0%, #0D1420 100%);
...
.exec-card::before {
    ...
    height: 2px;
    background: linear-gradient(90deg, #C8974A 0%, #C8974A 50%, #C8974A 100%);
...
_BASE = dict(
    ...
    xaxis=dict(
        gridcolor="#243347", zeroline=False,
        showspikes=True, spikecolor="#C8974A",
        spikethickness=1, spikedash="dot",
    ),
    yaxis=dict(gridcolor="#243347", zeroline=False),
    dragmode="zoom",
)
...
def _layout(title: str, sub: str = "", height: int = 420) -> dict:
    ...
    d["title"] = dict(
        text=(
            f"<span style='font-family:Cormorant Garamond,serif;font-size:14px;"
            f"font-weight:700;color:#EDE8DE'>{title}</span>"
...
def _show_profile(meta: dict):
    ...
    freq_color = "#6BAF85" if conf >= 0.8 else "#f59e0b"
    st.markdown(f"""
    <div class="profile-strip">
      ...
      <div class="p-item">
        ...
        <div class="p-value" style="color:{'#C45858' if dupes else '#6BAF85'}">{dupes}</div>
      </div>
      <div class="p-item">
        <div class="p-label">Gaps</div>
        <div class="p-value" style="color:{'#f59e0b' if gaps else '#6BAF85'}">{gaps}</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
def _data_intelligence(hist_df: pd.DataFrame):
    ...
    with t1:
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="tozeroy", fillcolor="rgba(200,151,74,0.05)",
...
def _rolling(hist_df: pd.DataFrame):
    ...
    fig.add_trace(go.Scatter(x=df["date"], y=df["value"], mode="lines",
        name="Actual", line=dict(color="#4A6278", width=1.5)), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(x=df["date"], y=df["R12"], mode="lines",
        name="12-Period MA", line=dict(color="#C8974A", width=2, dash="dash")), row=1, col=1)

    bar_colors = ["#6BAF85" if v >= 0 else "#C45858" for v in df["MoM"].fillna(0)]
    ...
    fig.update_xaxes(gridcolor="#243347", zeroline=False)
    fig.update_yaxes(gridcolor="#243347", zeroline=False)
    st.plotly_chart(fig, use_container_width=True)
...
def _seasonality(hist_df: pd.DataFrame):
    ...
    palette = ["#C8974A","#C8974A","#3b82f6","#60a5fa","#93c5fd",
               "#E2B96A","#C8974A","#6d28d9","#4c1d95","#ddd6fe"]
    ...
    if len(years) > 1:
        ...
        hfig = go.Figure(go.Heatmap(
            ...
            colorscale=[[0,"#0f2040"],[0.5,"#C8974A"],[1,"#93c5fd"]],
...
def _distribution(hist_df: pd.DataFrame):
    ...
    with c1:
        fig = go.Figure(go.Histogram(
            ...
            marker=dict(color="#C8974A", opacity=0.75,
                        line=dict(color="#C8974A", width=0.5)),
        ))
    ...
    with c2:
        fig2 = go.Figure(go.Box(
            y=vals, name="Values",
            marker_color="#C8974A", line_color="#60a5fa",
            fillcolor="rgba(200,151,74,0.12)",
    ...
    for lbl, val in stats:
        html += (
            f'<div style="background:#0D1420;border:1px solid #243347;border-radius:8px;'
            ...
            f'<div style="font-family:Cormorant Garamond,serif;font-size:.9rem;font-weight:600;'
            f'color:#8FA3B8">{_fmt(val)}</div></div>'
        )
    html += "</div>"
...
def _hero_chart(hist_df: pd.DataFrame):
    ...
    if pe is not None and pe_ci is not None:
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="toself", fillcolor="rgba(200,151,74,0.09)",
    ...
    fig.add_trace(go.Scatter(
        x=hist_df["date"], y=hist_df["value"],
        mode="lines", name="Actual",
        line=dict(color="#4A6278", width=1.5),
    ...
    if se is not None:
        fig.add_trace(go.Scatter(
            ...
            line=dict(color="#E2B96A", width=2, dash="dot"),
    ...
    fig.add_shape(type="line",
        ...
        xref="x", yref="paper",
        line=dict(color="#243347", width=1.5, dash="dash"),
    )
    fig.add_annotation(
        x=origin_str, y=1, xref="x", yref="paper",
        text="Forecast Origin",
        showarrow=False, yanchor="bottom",
        font=dict(size=9, color="#4A6278", family="DM Mono, monospace"),
    )
    ...
    with t1:
        ...
        if cert_df is not None and len(cert_df) > 0:
            ...
            _ai_insight(
                ...
                f"MASE < 0.70 (beats naive by 30%+): {elite_n}. MASE 0.70–0.85: {strong_n}. "
        ...
        if tier_col and "_tier_simple" in cert_df.columns:
            ...
            tc = tc[tc > 0]
            if not tc.empty:
                color_map = {"Elite":"#6BAF85","Strong":"#f59e0b",
                             "Pass":"#f97316","Fail":"#C45858"}
                bar = go.Figure(go.Bar(
                    x=tc.index.tolist(), y=tc.values.tolist(),
                    marker_color=[color_map.get(t,"#4A6278") for t in tc.index],
        ...
        if not forecasts:
            ...
            for model, series in forecasts.items():
                if model == "Primary Ensemble":
                    ...
                    lp = dict(color="#E2B96A", width=2, dash="dot")
                    vis = True
                else:
                    lp = dict(width=1, color="#243347")
                    vis = "legendonly"
                fig.add_trace(go.Scatter(
...
def _decomposition(hist_df: pd.DataFrame):
    ...
    rc = ["#6BAF85" if v >= 0 else "#C45858" for v in residual.fillna(0)]
    ...
    fig.add_trace(go.Histogram(x=residual.dropna().values, nbinsx=20,
        name="Residual Dist",
        marker=dict(color="#C8974A", opacity=0.7)), row=3, col=1)
...
def _scenario_overlay(hist_df: pd.DataFrame):
    ...
    up_col    = "#6BAF85" if direction == "upside" else "#C45858"
