...
def _forecast_chart(entity: str, ts: np.ndarray, result: dict,
    ...
    fig.add_trace(go.Scatter(
        ...
        fill="toself",
        fillcolor="rgba(201,148,58,0.10)",
        line=dict(width=0),
        hoverinfo="skip",
        name="95% CI",
    ))
...
def _render_outputs(entity_names, df_wide, month_labels, start_month):
    ...
    for nm in ordered_entities:
        ...
        with st.expander(f"📊 {nm}", expanded=(nm == ordered_entities[0])):
            tab_fc, tab_data, tab_yoy = st.tabs([
                "12-Month Forecast",
                "Forecast Data",
            ...
            with tab_data:
                ...
                # ── Section 1: 36-month actuals ──────────────────────────────
                st.markdown("##### 36-Month Historical Actuals")
                actual_rows = []
                for i, val in enumerate(ts):
                    mo_label = month_labels[i] if i < len(month_labels) else f"M{i+1}"
                    yr_label = f"Year {i // 12 + 1}"
                    actual_rows.append({
                        "#":     i + 1,
                        "Period": mo_label,
                        "Year":  yr_label,
                ...
                actuals_df["MoM Chg %"] = actuals_df["Actual"].pct_change().mul(100).round(2)
                # Add YoY % change (same month last year)
                actuals_df["YoY Chg %"] = (
                    actuals_df["Actual"]
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "MoM Chg %": st.column_config.NumberColumn(format="%.2f %%"),
                        "YoY Chg %": st.column_config.NumberColumn(format="%.2f %%"),
                    },
                )

                st.divider()

                # ── Section 2: 12-month forecast table ───────────────────────
                st.markdown("##### 12-Month Forward Forecast")

                # Compute error metrics from backtest window (last 12 of ts)
                backtest_h = 12
                if len(ts) >= backtest_h + 2:
                    bt_actual = ts[-backtest_h:]
                    # Use ETS backtest if available, else primary fc as proxy
                    if res.get("ets") and res["ets"].get("mase") is not None:
                        bt_mase = res["ets"]["mase"]
                    else:
                        bt_mase = primary.get("mase", np.nan)

                    bt_mae  = float(np.nanmean(np.abs(bt_actual - np.mean(bt_actual))))
                    bt_rmse = float(np.sqrt(np.nanmean((bt_actual - np.mean(bt_actual)) ** 2)))
                else:
                    bt_mase = primary.get("mase", np.nan)
                    bt_mae  = np.nan
                    bt_rmse = np.nan
                ...
                fc_rows = []
                for i in range(12):
                    fc_val  = float(fc_arr[i])
                    lo_val  = float(lo_arr[i])
                    hi_val  = float(hi_arr[i])
                    ci_width = hi_val - lo_val
                    ci_pct   = (ci_width / abs(fc_val) * 100) if fc_val != 0 else np.nan

                    # MoM change within forecast
                    if i == 0:
                        mom_pct = (fc_val - ts[-1]) / abs(ts[-1]) * 100 if ts[-1] != 0 else np.nan
                    else:
                        prev = float(fc_arr[i - 1])
                        mom_pct = (fc_val - prev) / abs(prev) * 100 if prev != 0 else np.nan

                    fc_rows.append({
                        "Period":      f"F{i+1}",
                        "Forecast":    round(fc_val, 2),
                        "Lower 95%":  round(lo_val, 2),
                        "Upper 95%":  round(hi_val, 2),
                        "CI Width":   round(ci_width, 2),
                        "CI %":       round(ci_pct, 1) if not np.isnan(ci_pct) else None,
                        "MoM Chg %":  round(mom_pct, 1) if not np.isnan(mom_pct) else None,
                    })
                ...
                totals_row = pd.DataFrame([{
                    "Period":     "TOTAL",
                    "Forecast":   round(float(np.nansum(fc_arr)), 2),
                    "Lower 95%":  round(float(np.nansum(lo_arr)), 2),
                    "Upper 95%":  round(float(np.nansum(hi_arr)), 2),
                    "CI Width":   round(float(np.nansum(hi_arr)) - float(np.nansum(lo_arr)), 2),
                    "CI %":       None,
                    "MoM Chg %":  None,
                }])
                fc_table_df = pd.concat([fc_table_df, totals_row], ignore_index=True)
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "CI Width":   st.column_config.NumberColumn(format="%.2f"),
                        "CI %":       st.column_config.NumberColumn(format="%.1f %%"),
                        "MoM Chg %":  st.column_config.NumberColumn(format="%.1f %%"),
                    },
                )

                # ── Accuracy metrics strip ────────────────────────────────────
                st.markdown("##### Backtest Accuracy (12-Month Hold-Out)")
                m1, m2, m3, m4 = st.columns(4)
                m1.metric(
                    "MASE",
                    f"{bt_mase:.4f}" if not np.isnan(bt_mase) else "—",
                    help="< 1.0 beats seasonal naïve. Elite < 0.70.",
                )
                m2.metric(
                    "Model",
                ...
                m3.metric(
                    "FC 12-mo Total",
                    f"{float(np.nansum(fc_arr)):,.0f}",
                )
                # Hist avg for context
                hist_avg = float(np.nanmean(ts))
                m4.metric(
                    "Hist Monthly Avg",
                    f"{hist_avg:,.0f}",
                )
