...
/* Override: sidebar collapse/expand toggle must use Material Icons, not DM Mono */
...
.sb-key { font-size:.6rem; color:#4A6278; letter-spacing:.06em; }
.sb-val { font-size:.68rem; font-weight:600; color:#8FA3B8; }
...
def render_command_center():
    ...
    with st.sidebar:
        ...
        if macro_mode == "Tier A — FRED Regressors":
            st.markdown(
                '<div style="font-size:.6rem;color:#4A6278;margin:.15rem 0 .4rem">'
            ...
            st.markdown(
                '<div style="font-size:.58rem;color:#4A6278;margin-top:.4rem;'
                'padding:.4rem .5rem;border:1px solid #243347;border-radius:4px;">'
        ...
        # ── Tier B — Multipliers ──────────────────────────────────────────
            ...
            if tier_b_enabled:
                ...
                impact_color = "#6BAF85" if total_impact > 0 else "#C45858" if total_impact < 0 else "#4A6278"
                st.markdown(
                    f'<div style="background:#1B2A40;border:1px solid #243347;'
                    f'border-radius:8px;padding:.6rem;margin:.4rem 0;text-align:center">'
                    f'<div style="font-size:.56rem;color:#4A6278;letter-spacing:.1em;'
        ...
        if not forecast_ready:
            st.markdown(
                '<div style="font-size:.62rem;color:#4A6278;padding:.25rem 0">'
        ...
        if sig:
            ...
            st.markdown(
                '<div style="font-size:.62rem;color:#4A6278">No run signature yet.</div>',
                unsafe_allow_html=True,
            )
