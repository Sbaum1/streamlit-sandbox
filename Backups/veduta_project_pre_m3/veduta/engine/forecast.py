...
def run_forecast(
    ...
    backtest_strategy: str = "expanding",
    ensemble_weight_method: str = "mase",
) -> dict:
    """
    Run sentinel_engine and return a results bundle compatible
    with the VEDUTA session state contract.
    ...
    # ── Run sentinel engine ───────────────────────────────────────────────────
    # ── Backtest strategy: expanding (default) or rolling fixed window ───────
    from sentinel_engine.backtest import run_backtest as _base_backtest

    if backtest_strategy == "rolling":
        # Rolling: fix training window size = len(df) - forecast_horizon * folds
        _n = len(df)
        _folds = 3
        _fixed_window = max(36, _n - forecast_horizon * _folds)
        def _rolling_backtest_fn(df_bt, model_runner, horizon, confidence_level, **kw):
            # Slice to fixed window before each fold evaluation
            import functools
            def _windowed_runner(df_inner, horizon, confidence_level):
                sliced = df_inner.tail(_fixed_window).reset_index(drop=True)
                return model_runner(sliced, horizon, confidence_level)
            return _base_backtest(
                df=df_bt, model_runner=model_runner,
                horizon=horizon, confidence_level=confidence_level,
                rolling_folds=_folds,
            )
        backtest_fn = _rolling_backtest_fn
    else:
        # Expanding: standard walk-forward (engine default)
        backtest_fn = None  # engine uses its default _default_backtest
