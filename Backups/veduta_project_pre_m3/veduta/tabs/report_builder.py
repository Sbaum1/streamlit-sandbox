# ==================================================
# FILE: veduta/tabs/report_builder.py
# VERSION: 2.0.0
# ROLE: REPORT BUILDER / EXPORT STUDIO
# UPDATED: Phase 4 — Foresight engine metadata,
...
""", unsafe_allow_html=True)
    ...
    st.download_button(
        ...
        file_name="veduta_committed_data.csv",
        mime="text/csv",
    )
    ...
    if cert:
        st.download_button(
            ...
            file_name="veduta_certification_results.csv",
            mime="text/csv",
        )
    ...
    if st.button("Generate Full Excel Workbook"):
        with tempfile.TemporaryDirectory() as tmpdir:
            path = os.path.join(tmpdir, "VEDUTA_Foresight_Report.xlsx")
            export_excel(package, path)
            with open(path, "rb") as f:
                st.download_button(
                    label="⬇️  Download Excel Workbook",
                    data=f.read(),
                    file_name="VEDUTA_Foresight_Report.xlsx",
