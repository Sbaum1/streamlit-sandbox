...
def render():
    ...
    if st.session_state.get("dataset_committed", False):
        ...
        # --------------------------------------------------
        # RUN FORECAST (CERTIFICATION-INTEGRATED)
        # --------------------------------------------------
            ...
            if st.button("Run Forecast"):
                try:
                    ...
                    if execution_mode == "Executive Default Mode":
                        ...
                        if "Primary Ensemble" not in all_results:
                            raise RuntimeError(
                                "Primary Ensemble missing from registry execution."
                            )

                        results = {
                            "Primary Ensemble": all_results["Primary Ensemble"]
                        }
