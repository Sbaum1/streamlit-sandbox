...
# - Ported from streamlit_sandbox/analysis/backtest_engine.py
# - Fully deterministic and seasonally correct
...
# - ForecastResult identity uses sentinel_engine.contracts exclusively
#
# UPGRADES vs ORIGINAL:
# - ROLLING_FOLDS configurable (default 3)
# - SEASONAL_PERIOD configurable (default 12)
# - CI all-NaN guard — avoids reporting 0.0 coverage on NaN intervals
# - engine_version in output
# - forecast column cast to float64 before evaluation
# - ci_low / ci_high cast to float64 before coverage computation
...
MIN_OBSERVATIONS = 36
DEFAULT_ROLLING_FOLDS  = 3
DEFAULT_SEASONAL_PERIOD = 12

# G7: Minimum scale floor for MASE denominator.
# Prevents division by zero and astronomically large MASE scores
# on near-flat or zero-mean series. Value chosen to be below any
# meaningful real-world forecast error but above floating-point noise.
...
def _mase(
    ...
    # G7: Clamp to floor — handles exact zero and near-zero series
    scale = float(max(scale, MASE_SCALE_FLOOR))
